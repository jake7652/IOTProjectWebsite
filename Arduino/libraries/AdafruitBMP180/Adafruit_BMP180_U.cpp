...
static void read8(byte reg, uint8_t *value)
{
  ...
  #endif
  Wire.endTransmission();
  Wire.requestFrom((uint8_t)BMP180_ADDRESS, (byte)1);
  #if ARDUINO >= 100
    *value = Wire.read();
  #else
    *value = Wire.receive();
...
static void read16(byte reg, uint16_t *value)
{
  ...
  #endif
  Wire.endTransmission();
  Wire.requestFrom((uint8_t)BMP180_ADDRESS, (byte)2);
  #if ARDUINO >= 100
...
static void readRawPressure(int32_t *pressure)
{
    ...
    *pressure = 23843;
    ...
    writeCommand(BMP180_REGISTER_CONTROL, BMP180_REGISTER_READPRESSURECMD + (_BMP180Mode << 6));
    switch(_BMP180Mode)
    {
...
/**************************************************************************/
/*!
    @brief  Instantiates a new Adafruit_BMP180_Unified class
*/
/**************************************************************************/
...
void Adafruit_BMP180_Unified::getPressure(float *pressure)
{
  ...
  x1 = (ut - (int32_t)(_BMP180_coeffs.ac6))*((int32_t)(_BMP180_coeffs.ac5))/pow(2,15);
  x2 = ((int32_t)(_BMP180_coeffs.mc*pow(2,11)))/(x1+(int32_t)(_BMP180_coeffs.md));
  b5 = x1 + x2;
...
*/
/**************************************************************************/
float Adafruit_BMP180_Unified::pressureToAltitude(float seaLevel, float atmospheric, float temp)
{
...
*/
/**************************************************************************/
void Adafruit_BMP180_Unified::getEvent(sensors_event_t *event)
{
  float pressure_kPa;
